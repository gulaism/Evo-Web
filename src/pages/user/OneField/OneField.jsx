...
import { useGetCourseDetailsQuery } from "../../../redux/services/apiSlice";
...
//   },
// ];
const Item = ({ header, span, description }) => {
  const splitTitle =
    span && header
      ? header.split(new RegExp(`(${span})`, "i"))
      : [header]; // If span is null, keep title as is
...
const OneField = () => {
  ...
  const { data: field, isError: isFieldError, isLoading: isFieldLoading } = useGetCourseDetailsQuery(fieldName);
  ...
  // }
  useEffect(() => {
    if (!isFieldError && !isFieldLoading) {
      console.log(field);
    }
  }, [isFieldError, isFieldLoading])
  ...
  const rows = Math.ceil(field?.whatYouLearnItems.length / itemsPerRow);
      ...
      <div className="container">
        <section className={styles.firstSection}>
          ...
          <div className={styles.generalInfo}>
            <div className={styles.fieldName}>{field?.areaName}</div>
            <div className={styles.fieldDesc}>
              {field?.areaDescription.map((desc, index) => (
        ...
        <section className={styles.whatYouWillGet}>
          ...
          <div className={styles.grid}>
            {Array.from({ length: rows }).map((_, rowIndex) => {
              ...
              const end = Math.min(
                start + itemsPerRow,
                field?.whatYouLearnItems.length
              );
              const rowItems = field?.whatYouLearnItems.slice(start, end);
        ...
        <section className={styles.program}>
          ...
          <div className={styles.programDuration}>
            <div>
              <div className={styles.number}>{field?.lessonDuration}</div>
              <div className={styles.detail}>Ay</div>
            </div>
            <div>
              <div className={styles.number}>{field?.lessonHours}</div>
              <div className={styles.detail}>Saat</div>
            </div>
            <div>
              <div className={styles.number}>{field?.moduleCount}</div>
              <div className={styles.detail}>Modul</div>
            </div>
          </div>
          <div className={styles.question}>Hansı alətlərlə işləyəcəksən?</div>
          <div className={styles.tools}>
            {field?.technologies.map((tool, index) => (
          ...
          <div className={styles.grid}>
            {field?.modules.map((mod, index) => (
              <div key={index}>
                <div className={styles.title}>{mod.moduleName}</div>
                <div className={styles.description}>{mod.moduleDescription}</div>
              </div>
            ))}
          </div>
        </section>
                ...
                <div className={styles.animationCont}>
                  <div className={styles.arrowWrapper}>
                    ...
                    <a
                      ...
                      href={`https://api.whatsapp.com/send/?phone=${field?.contactPhone.replace(/\D/g, "")}&text&type=phone_number&app_absent=0`}
                    ></a>
                  </div>
                </div>
              </div>
